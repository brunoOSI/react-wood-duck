...
class NavLink extends React.Component {
   ...
   render() {
      ...
      const activeNavLinkClassName = this.props.active ? 'active-navlink' : 'inactive-navlink';
      const activeAnchorClassName = this.props.active ? 'active' : '';
      const indentationStyle={marginRight: this.props.indentationLevel * 3 + 'rem' };
      return(
         <li className='navlink' key={this.props.text} >
           <span className={activeNavLinkClassName} style={indentationStyle}/>
           {preIcon}<a href={this.props.href} className={activeAnchorClassName} onClick={(e) => this.props.handleClick(this.props.href, e)}>{this.props.text}</a>{postIcon}
           {this.props.children}
         </li>
      );
   }
