...
describe('NavLink', function() {
   ...
   var inactiveBasicNavLink = ReactTestUtils.renderIntoDocument(<NavLink href={navLinkHref} text={navLinkText} indentationLevel={indentationLevel} active={false} onClick={function(){console.log('test function');}}/>)
   var navLinkWithPreIcon = ReactTestUtils.renderIntoDocument(<NavLink href={navLinkHref} text={navLinkText} preIcon={preIconClass} />)
   var activeNavLinkWithPreIcon = ReactTestUtils.renderIntoDocument(<NavLink href={navLinkHref} text={navLinkText} preIcon={preIconClass} active={true}/>)
   ...
   const navLinkWithChildren = ReactTestUtils.renderIntoDocument(<NavLink href={navLinkHref} text={navLinkText}><div>Test Child</div></NavLink>)
   it('verify basic navlink has li element', function() {
      expect(inactiveBasicNavLink).not.toBe(null)
      expect(ReactTestUtils.isCompositeComponent(inactiveBasicNavLink, NavLink)).toBe(true)
      const liElement = ReactTestUtils.findRenderedDOMComponentWithTag(inactiveBasicNavLink, 'li')
   ...
   it('verify basic navlink has span element with inactive class name and valid indentation style', function() {
      const liElement = ReactTestUtils.findRenderedDOMComponentWithTag(inactiveBasicNavLink, 'li')
      const spanElement = ReactTestUtils.findRenderedDOMComponentWithTag(inactiveBasicNavLink, 'span')
   ...
   it('verify navlink renders with pre Icon & active style class', function() {
     const liElement = ReactTestUtils.findRenderedDOMComponentWithTag(activeNavLinkWithPreIcon, 'li')
     expect(liElement.children.length).toBe(3)
     expect(liElement.children[0].tagName).toBe('span'.toUpperCase())
